...
Somente uma resposta HTTP 2xx com JSON contendo `"allow": true` é aceita como autorização. Qualquer outra resposta, timeout, erro TLS, JSON inválido ou campo ausente resulta em `false`. A API deve validar o segredo e decidir permissões a partir de dados confiáveis; não basta devolver `allow` indiscriminadamente.
