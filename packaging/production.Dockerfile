...
WORKDIR /src
RUN git clone https://github.com/rustdesk/rustdesk-server.git . && git checkout 9bae9f2f39d92c4b4ba2e28e089da5071897b22e && git submodule update --init --recursive
COPY server.patch /tmp/server.patch
RUN git apply --check /tmp/server.patch && git apply /tmp/server.patch
